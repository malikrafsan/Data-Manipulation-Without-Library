...
def mintaConsumable():
    ...
    while syrt:
        try:
            ...
            if amount_asked > 0 and amount_asked <= consumable[j][3]:
                syrt = False
            if syrt == True:
                print(f"Silahkan masukan jumlah dengan benar, minimal 1 dan maksimal {consumable[j][3]}")
        except ValueError:
    ...
    consumable [j][3] = consumable[j][3] - amount_asked

    for idks in range(len(inventory_user)):
        if ID == inventory_user[idks][1] and idUser == inventory_user[idks][0]:
            inventory_user[idks][2] = inventory_user[idks][2] + amount_asked
            break
